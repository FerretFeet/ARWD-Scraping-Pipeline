...
class Orchestrator:
    """Orchestrates the main process, creates and manages threads and state."""
    ...
    def orchestrate(self) -> None:
        """Organize and manage threads and inputs."""
        ...
        logger.info(
            f"FETCH QUEUE SIZE AFTER LOADING: {self.queues[PipelineRegistries.FETCH].qsize()}",
        )
    ...
    def manage_workers(self, workers: list[BaseWorker], ordered_queues: list[Queue]) -> None:
        ...
        while any(urls for urls in self.seed_urls.values()):
            active_root_netlocs = {
                parse_url(node.url).netloc
                for node in self.state.get_roots()
    ...
    def _drain_sentinels(self, queues: dict) -> None:
    ...
    def _load_queues(self, unvisited_nodes: list[directed_graph.Node]) -> None:
        ...
        if unvisited_nodes:
            for node in unvisited_nodes:
                ...
                if node.state in [
                    PipelineStateEnum.CREATED,
                    PipelineStateEnum.AWAITING_FETCH,
                    PipelineStateEnum.FETCHING,
                ]:
                    root_queue.put(node)
                elif node.state in [
                    PipelineStateEnum.AWAITING_PROCESSING,
                    PipelineStateEnum.PROCESSING,
                ]:
    ...
    def organize_unique_domains(self, seed_urls: list[str]) -> dict[str, list[str]]:
        """Turn a raw list of urls into a dict of lists organized by domain."""
        url_dict = {}
    ...
    def setup_states(
        ...
        """Add nodes to state from seed urls and cache."""
        nodes_to_stack = {}
        addtl_nodes = []
        addtl_keys = []
        ...
        for key in seed_urls.copy():
            if key not in nodes_to_stack and key not in addtl_keys:
        ...
        return [
            item
            for items in zip_longest(nodes_to_stack.values(), addtl_nodes)
            for item in items
            if item is not None
        ]
