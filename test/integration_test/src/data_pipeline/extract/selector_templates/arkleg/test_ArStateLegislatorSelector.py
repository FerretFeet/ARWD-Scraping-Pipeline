...
def known_legislator_soup_fixture() -> BeautifulSoup:
    ...
    fixture_path = project_root / "test" / "fixtures" / "html" / "legislator" / "legislator.known.html"
...
class TestArStateLegislatorSelector:
    ...
    def test_known_legislator_known_return(self, mock_get_page, known_legislator_soup_fixture):
        ...
        assert result['public_service'] == ['Representative 2015,  2017,  2019,  2021,  Senate 2023,  2025']

        #committees too big to check all, just check first and last
        assert result['committees'][0].strip() == 'TASK FORCE ON AUTISM'
        assert result['committee_links'][0] == '/Committees/Detail?code=985&ddBienniumSession=2025%2F2025R'
        ...
        assert result['committee_links'][29] == '/Committees/Detail?code=015&ddBienniumSession=2025%2F2025R'
