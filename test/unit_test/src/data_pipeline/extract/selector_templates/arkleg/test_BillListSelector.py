...
def known_bill_list_soup_fixture() -> BeautifulSoup:
    ...
    fixture_path = project_root / "test" / "fixtures" / "html" / "bill_list_page" / "bill_list.known.html"
...
class TestArStateLegislatorSelector:
    ...
    def test_known_bill_list_known_return(self, mock_get_page,
                                              known_bill_list_soup_fixture):
        ...
        assert len(result['bill_url']) == 20
        assert result['bill_url'][0] == '/Bills/Detail?id=HB1001&ddBienniumSession=2025%2F2025R'
        assert result['next_page'] == ['?start=20&type=HB&ddBienniumSession=2025%2F2025R#SearchResults']
